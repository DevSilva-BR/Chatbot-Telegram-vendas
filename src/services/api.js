...
class MercadoPagoAPI {
  ...
  async createPayment(ctx, amount, cpf, email,userId, userName, nick) {
    try {
      ...
      console.log("valor do", userId);
      const order = await paymentService.savePaymentOrder(
        response,
        userId,
        userName,
        nick,
      ...
      console.log("valor da order:", order);

      await this.handleQRCode(ctx, response.data, idempotencyKey);
      return order;
    } catch (error) {
      console.error(
  ...
  async handleQRCode(ctx, response, idempotencyKey) {
    const paymentData = response; // Já é response.data
    if (
      !paymentData ||
  ...
  async saveOrderDetails(ctx, response, email, name, cpf, userId) {
    try {
      console.log("dados",email,name,cpf, userId)
